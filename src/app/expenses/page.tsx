...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    ...
    setTransactions(storedTransactions.filter((t: Transaction) => t.type === 'expense'));
  }, []);
  ...
  const handleUpdateTransaction = (updatedTransaction: Omit<Transaction, 'id' | 'type'>) => {
    ...
    const updatedTransactions = storedTransactions.map((t: Transaction) => 
      t.id === editingTransaction?.id 
        ? { ...updatedTransaction, id: editingTransaction.id, type: 'expense' } 
        : t
    );
    
    localStorage.setItem('transactions', JSON.stringify(updatedTransactions));
    setTransactions(updatedTransactions.filter((t: Transaction) => t.type === 'expense'));
    setIsFormOpen(false);
    setEditingTransaction(null);
  };
